...
SELECT 
extract (year from f.order_date) AS order_year, p.product_name
...
SELECT category, total_sales
    ...
    , ROUND( (total_sales::numeric / SUM(total_sales) OVER () ) * 100, 2 )
FROM category_sales
ORDER BY 2 DESC
...
EXTRACT(YEAR FROM AGE(NOW(), last_order_date)) AS age2,
total_orders,
total_sales,
total_quantity,
total_products
...
--	DATEDIFF(MONTH, last_sale_date, GETDATE()) AS recency_in_months,
	, EXTRACT(year FROM AGE(NOW(), last_sale_date)) * 12 + EXTRACT(month FROM AGE(NOW(), last_sale_date)) AS recency_in_months
	, CASE
